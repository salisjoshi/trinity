...
/*******START: CUSTOM ************/
/*********************************/
//new element added to track WC packet trapping
struct rx_stats
{
	...
	//unsigned long rx_wc_ecn_bytes_trap;
...
{
	printk(KERN_INFO "inside rx init");
	if(likely(ptr!=NULL))
	{
		printk(KERN_INFO "ptr is not null inside rx init");
		ptr->endpoint_num=0;
...
static unsigned int Init_pair_rx_context(struct pair_rx_context* ptr, unsigned int local_ip, unsigned int remote_ip, unsigned int bw)
{
	if(likely(ptr!=NULL))
	{
		...
		ptr->stats.last_update_time=now;	//Added to track packet trapping
...
static void Empty_rx_context(struct rx_context* ptr)
{
	...
	else // bug corrected, no else statement
	{
		printk(KERN_INFO "Error: NULL pointer\n");
	}
}
