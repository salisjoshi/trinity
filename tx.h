...
/*******START: CUSTOM ************/
/*********************************/
//New structure added to track WC packet trapping
struct tx_stats
{
	unsigned long tx_wc_ecn_bytes_trap;
	ktime_t last_update_time;
	ktime_t last_data_sent_time;
};
/*********************************/
/*******END: CUSTOM **************/
/*********************************/
...
struct pair_tx_context 
{
	...
	struct tx_stats stats;			//Element added to track WC packet trapping	
...
struct endpoint_tx_context
{
	...
	//We use tasklet(softirq) rather than hardirq context of hrtimer to implement rate limiting
	struct tasklet_struct tx_xmit_timeout;
	//Timer for rate limiting
	struct hrtimer timer;
	//Timer granularity (us)
	unsigned int timer_interval;
...
static unsigned int Init_endpoint_tx_context(struct endpoint_tx_context* ptr, unsigned int ip, unsigned int bw,void (*tasklet_func)(unsigned long), enum hrtimer_restart  (*timer_func)(struct hrtimer *), unsigned int delay)
{
	if(likely(ptr!=NULL))
	{
		ptr->pair_num=0;
		...
		/*******START: CUSTOM ************/
		/*********************************/
		//Code added to initialize timer to calculate unutilized WC rate periodically
		//Initialize tasklet 
		tasklet_init(&(ptr->tx_xmit_timeout), *tasklet_func, (unsigned long)ptr);
		//Initialize hrtimer
		hrtimer_init(&(ptr->timer), CLOCK_MONOTONIC, HRTIMER_MODE_REL);
		ptr->timer.function=timer_func;
		ptr->timer_interval=delay;
		hrtimer_start(&(ptr->timer), ktime_set(delay, 0), HRTIMER_MODE_REL);
		/*********************************/
		/*******END: CUSTOM **************/
		/*********************************/
	...
	int flags)
{
	...
	ptr->guarantee_bw=bw;
	ptr->stats.tx_wc_ecn_bytes_trap=0;
	ptr->stats.last_update_time=ktime_get();
	ptr->stats.last_data_sent_time=ktime_get();
	INIT_LIST_HEAD(&(ptr->list));

	if(unlikely(Init_dual_tbf(&(ptr->rateLimiter),bw,0,bucket,bucket,max_len,flags)==0))
	{
		return 0;
	}
	...
	ptr->timer.function=timer_func;
	ptr->timer_interval=delay;
	hrtimer_start(&(ptr->timer), ktime_set(0, delay ), HRTIMER_MODE_REL);
	//Initialize spinlock
...
static void Insert_tx_pair(struct pair_tx_context* pair_ptr, struct tx_context* ptr)
{
	if(likely(pair_ptr!=NULL&&ptr!=NULL))
	{
		...
		list_for_each_entry(endpoint_ptr,&(ptr->endpoint_list),list)
		{
			...
			printk(KERN_INFO "Insert TX Pair:List found");
			if(endpoint_ptr->local_ip==pair_ptr->local_ip)
			{
				printk(KERN_INFO "Insert TX Pair:ip matched");
...
static void Empty_tx_context(struct tx_context* ptr)
{
	if(likely(ptr!=NULL))
	{
		...
		list_for_each_entry_safe(endpoint_ptr, endpoint_next, &(ptr->endpoint_list), list)
		{
			...
			hrtimer_cancel(&(endpoint_ptr->timer));
			tasklet_kill(&(endpoint_ptr->tx_xmit_timeout));
